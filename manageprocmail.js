  if (window.rcmail) {
    ...
    if (rcmail.env.action.startsWith('plugin.manageprocmail')) {
      ...
      // alert('ok');
        ...
        rcmail.filters_list
          .addEventListener('select', function(e) { rcmail.manageprocmail_select(e); })
          ...
          .addEventListener('initrow', function(row) {
            row.obj.onmouseover = function() { rcmail.manageprocmail_focus_filter(row); };
            row.obj.onmouseout = function() { rcmail.manageprocmail_unfocus_filter(row); };
          })
          .init();
      }
    }
